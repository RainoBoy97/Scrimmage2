...
public class UserCommands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String alias,
		...
		// JOIN
		/*
		 * if (cmd.getName().equalsIgnoreCase("join")) { Team oldteam; Team
		 * team; boolean obs = false; oldteam = TeamHandler.getTeam((Player)
		 * sender); if (args.length == 1) { team = Team.getTeam(args[0]); } else
		 * { team = TeamHandler.getTeamWithLessPlayers(); } if (team ==
		 * Team.OBSERVER) { obs = true; } if (MatchHandler.played() && !obs) {
		 * Scrimmage.msg(player, ChatColor.RED + "Match is over - " +
		 * ChatColor.DARK_AQUA +
		 * "You cannot join a team until the next match!"); return true; } if
		 * (team == null) { Scrimmage.msg(player, ChatColor.RED +
		 * "Invalid team!"); return true; } if (th.isJoined(player) && !obs) {
		 * Scrimmage.msg(player, ChatColor.RED +
		 * "You have already joined a team!"); return true; }
		 * TeamHandler.addPlayer(player, team); Scrimmage .msg(player,
		 * ChatColor.GRAY + "You joined team " + th.getTeamColor(team) +
		 * StringUtils.capitalize(team.name() .toLowerCase()));
		 * Bukkit.getPluginManager().callEvent( new ScrimTeamJoinEvent(player,
		 * team, oldteam, th)); if (team == Team.OBSERVER) {
		 * Bukkit.getPluginManager().callEvent( new
		 * ScrimObsFromTeamJoinEvent(player, oldteam, th)); } else {
		 * Bukkit.getPluginManager().callEvent( new ScrimPvpJoinEvent(player,
		 * team, th)); }
		 * 
		 * }
		 */
		 ...
		 * player.sendMessage(ChatColor.DARK_AQUA + "No match running!"); } else
		 ...
		 * th.count(Team.RED) + ChatColor.DARK_AQUA + " | " + ChatColor.BLUE +
		 * "Blue: " + ChatColor.GRAY + th.count(Team.BLUE) + ChatColor.DARK_AQUA

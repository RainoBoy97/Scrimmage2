				...
				if (count != 0) {
					if (count % 5 == 0 || count <= 5 && count > 1) {
						Bukkit.broadcastMessage(ChatColor.GREEN
								+ "Game starting in " + ChatColor.RED + count
	...
	public static void startCycle(final int seconds) {
		...
		tasks.add(new BukkitRunnable() {
			...
			public void run() {
				if (count != 0) {
					if (count % 5 == 0 || count <= 5 && count > 1) {
						Bukkit.broadcastMessage(ChatColor.GREEN + "Cycling to "
								+ ChatColor.DARK_AQUA + next.getName()
								+ ChatColor.GREEN + " in " + ChatColor.RED
								+ count + ChatColor.GREEN + " seconds!");
					} else if (count == 1) {
						Bukkit.broadcastMessage(ChatColor.GREEN + "Cycling to "
								+ ChatColor.DARK_AQUA + next.getName()
								+ ChatColor.GREEN + " in " + ChatColor.RED
								+ count + ChatColor.GREEN + " second!");
					}
				} else if (count == 0) {
					Bukkit.broadcastMessage(ChatColor.GREEN + "Now playing "
							+ ChatColor.DARK_AQUA + next.getName()
							+ ChatColor.GREEN + "!");
					cancel();
				}
